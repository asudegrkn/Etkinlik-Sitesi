...
# def change_url_blocker(f):
#     @wraps(f)
#     def decorated_function(*args, **kwargs):
#         cursor = mysql.connection.cursor()
#         sorgu = "Select id from kullanici"
...
def kullaniciprofili():
    ...
    sorgu2 = "select DISTINCT e.id, e.event_name, e.event_time, e.event_place, e.event_speakers from etkinlikler e, kullanicilar k, etkinlik_katilimci ek where e.id in (Select e.id from etkinlik_katilimci where ek.etkinlik_id in (Select ek.etkinlik_id from etkinlik_katilimci where ek.katilimci_id = %s))"
...
def sertifikagoruntule(eid):
    cursor = mysql.connection.cursor()
    cursor2 = mysql.connection.cursor()
    sorgu = "select DISTINCT k.name, k.surname, k.major, e.event_time, e.event_company, e.event_name, e.event_speakers, e.event_photo, e.event_department from etkinlikler e, kullanicilar k, etkinlik_katilimci ek where k.id = %s and e.id = %s"
    ...
    if result >= 0 :
        ...
        if bolumler.endswith(","):
            ...
            bolumler = bolumler
        sorgu2 = "select DISTINCT e.event_company, e.event_name, e.event_time, e.event_speakers,e.event_photo, e.event_department from etkinlikler e, kullanicilar k, etkinlik_katilimci ek where e.id in (Select ek.etkinlik_id from etkinlik_katilimci where ek.katilimci_id = %s);"
        cursor2.execute(sorgu2,(session["id"],))
        data2 = cursor2.fetchall()
        return render_template("sertifikagoruntule.html", data=data[0], data2 = data2, bolumler = bolumler)
    else:
        return render_template("sertifikagoruntule.html", data=data[0])
